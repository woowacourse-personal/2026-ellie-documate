...
export function translationSystem(opts: { docTitle?: string; context?: string } = {}): string {
  ...
  // 규칙 개수·순서가 품질을 좌우한다: 규칙이 많거나 자연스러움 지시가 뒤로 가면 flash-lite가
  // 직역으로 되돌아간다(실측: 7번에 뒀더니 "다시 그립니다"→"리컴포지션이 예약됩니다"로 회귀).
  // → 자연스러움을 역할 바로 다음(2번) 최우선으로 두고, 나머지는 압축 유지.
  // JSON 형식 줄은 프록시 파서가 의존하는 기술 계약이라 유지한다(없으면 길이 불일치 → 502,
  // 특히 responseSchema 없이 도는 폴백 경로에서 치명적).
  const lines = [
