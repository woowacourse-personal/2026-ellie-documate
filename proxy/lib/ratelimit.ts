...
//
// 한도 산정:
//   과거 무료 티어(500 RPD·API 키 하나 공유)일 땐 DAILY 100/일로 잡아 한 명이 종일
//   태우는 걸 막았다. 결제 활성화(유료 standard 티어) 후 그 500 RPD 상한이 사라져
//   DAILY 100은 근거를 잃고 개발자 본인만 묶었다 → DAILY 10000/일로 상향.
//   한 페이지 정독 ≈ 번역 4요청(25문단 청크) + 해설 몇 번 ≈ 5~7요청.
//   BURST 20/10초 : 페이지 하나가 청크 4개를 병렬로 쏘므로 그보다 넉넉해야 한다.
//                   두드리기(hammering)를 막는 용도 — 유료 티어에서도 폭주 방어로 유지.
//   DAILY 10000/일: 1인당 하루 ~1500페이지. 사람이 그만큼 읽지 않는다 → 비용 폭탄·
//                   자동화 남용만 막는 상한. 실제 방어는 Google 지출 상한이 맡는다.
// 공용 캐시가 적중하면 Gemini를 안 부르므로 실질 여유는 이보다 훨씬 크다.
// 숫자는 실제 소모를 보고 조정한다.
const BURST = { limit: 20, window: '10 s' } as const;
const DAILY = { limit: 10000, window: '1 d' } as const;
